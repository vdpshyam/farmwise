...
class BuyerOrderScreenFlaggedOrderWidget extends StatelessWidget {
  const BuyerOrderScreenFlaggedOrderWidget({
    super.key,
    required this.order, required this.updateOrderListFunc,
  });

  final OrderScreenFlaggedOrderWidgetModel order;
  final Function updateOrderListFunc;
    ...
    http
      ...
      if (response.statusCode == 200) {
        ...
        if (markAsCompleteResp["message"] == "Order updated") {
          updateOrderListFunc(order.orderId);
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
